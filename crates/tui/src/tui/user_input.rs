...
impl ModalView for UserInputView {
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.mode == InputMode::OtherInput {
            lines.push(Line::from(vec![
                Span::styled("Enter", Style::default().fg(palette::DS_SKY).bold()),
                Span::styled(" submit", Style::default().fg(palette::TEXT_MUTED)),
                Span::raw("  "),
                Span::styled("Esc", Style::default().fg(palette::DS_SKY).bold()),
                Span::styled(" back", Style::default().fg(palette::TEXT_MUTED)),
            ]));
        } else {
            lines.push(Line::from(vec![
                Span::styled("1-4", Style::default().fg(palette::DS_SKY).bold()),
                Span::styled(" quick pick", Style::default().fg(palette::TEXT_MUTED)),
                Span::raw("  "),
                Span::styled("Up/Down", Style::default().fg(palette::DS_SKY).bold()),
