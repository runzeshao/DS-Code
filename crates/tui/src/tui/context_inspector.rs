...
use crate::models::{
    LEGACY_DS_CONTEXT_WINDOW_TOKENS, SystemPrompt, context_window_for_model,
};
...
fn context_usage(app: &App) -> (usize, u32, f64) {
    let max = context_window_for_model(&app.model).unwrap_or(LEGACY_DS_CONTEXT_WINDOW_TOKENS);
    let estimated =
