...
impl ViewStack {
    ...
    pub fn push<V: ModalView + 'static>(&mut self, view: V) {
        ...
        tracing::debug!(target: "DS_tui::view_stack", action = "push", kind = ?kind, depth = self.views.len(), "view pushed");
    }
    ...
    pub fn push_boxed(&mut self, view: Box<dyn ModalView>) {
        ...
        tracing::debug!(target: "DS_tui::view_stack", action = "push_boxed", kind = ?kind, depth = self.views.len(), "view pushed");
    }

    pub fn pop(&mut self) -> Option<Box<dyn ModalView>> {
        let popped = self.views.pop();
        if let Some(view) = popped.as_ref() {
            tracing::debug!(target: "DS_tui::view_stack", action = "pop", kind = ?view.kind(), depth = self.views.len(), "view popped");
        }
        popped
    }
        ...
        match action {
            ...
            ViewAction::Close => {
                if let Some(view) = self.views.pop() {
                    tracing::debug!(target: "DS_tui::view_stack", action = "close", kind = ?view.kind(), depth = self.views.len(), "view closed via action");
                }
            }
            ...
            ViewAction::EmitAndClose(event) => {
                events.push(event);
                if let Some(view) = self.views.pop() {
                    tracing::debug!(target: "DS_tui::view_stack", action = "emit_and_close", kind = ?view.kind(), depth = self.views.len(), "view closed via action");
                }
            }
        }
        events
    ...
    fn render(&self, area: Rect, buf: &mut Buffer) {
        ...
        if self.agents.is_empty() {
            ...
            let status_summary = [
                ...
                ("Failed", failed.len(), palette::DS_RED),
            ...
            .block(
                Block::default()
                    ...
                    .title_bottom(Line::from(vec![
                        ...
                        Span::styled(scroll_indicator, Style::default().fg(palette::DS_SKY)),
                    ]))
                    .borders(Borders::ALL)
                    .border_style(Style::default().fg(palette::BORDER_COLOR))
                    .style(Style::default().bg(palette::DS_INK))
    ...
    for agent in agents {
        ...
        if let Some(detail) = status_detail {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(detail, Style::default().fg(palette::DS_RED)),
            ]));
        }
        ...
        if let Some(role) = agent.assignment.role.as_deref() {
            ...
            lines.push(Line::from(vec![
                ...
                Span::styled(role, Style::default().fg(palette::DS_SKY)),
            ]));
        }
...
fn format_agent_status(
    ...
    match status {
        SubAgentStatus::Running => ("running", Style::default().fg(palette::DS_SKY), None),
        SubAgentStatus::Completed => (
            "completed",
            Style::default().fg(palette::DS_BLUE),
            None,
        ),
        ...
        SubAgentStatus::Failed(reason) => (
            "failed",
            Style::default().fg(palette::DS_RED),
            Some(reason.as_str()),
        ),
    }
}
