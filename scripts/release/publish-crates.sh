...
missing_packages=()
for workspace_package in "${workspace_DS_packages[@]}"; do
  found=0
...
package_has_workspace_deps() {
  ...
  local index
  for ((index = 0; index < ${#workspace_DS_packages[@]}; index += 1)); do
    if [[ "${workspace_DS_packages[$index]}" == "${package_name}" ]]; then
