...
#   2. `npm/DS-Code/package.json` `version` matches the workspace
#      `version` in the root `Cargo.toml`.
#   3. Internal `ds-*` path dependency pins match the workspace version.
...
npm_version="$(node -p "require('./npm/DS-Code/package.json').version")"
if [[ "${workspace_version}" != "${npm_version}" ]]; then
  echo "::error::npm/DS-Code/package.json version (${npm_version}) does not match workspace Cargo.toml (${workspace_version})." >&2
  fail=1
fi

# 3) Internal path dependency pins.
internal_dep_drift="$(
  grep -nE 'ds-[a-z-]+[[:space:]]*=[[:space:]]*\{[^}]*version[[:space:]]*=[[:space:]]*"' crates/*/Cargo.toml \
    | grep -v "version[[:space:]]*=[[:space:]]*\"${workspace_version}\"" || true
)"
if [[ -n "${internal_dep_drift}" ]]; then
  echo "::error::Internal ds-* path dependency versions must match workspace version ${workspace_version}:" >&2
...
if ! cargo metadata --locked --format-version 1 --no-deps >/dev/null 2>&1; then
  echo "::error::Cargo.lock is out of sync with the manifests. Run 'cargo update -p DS-Code' or 'cargo build' and commit the result." >&2
  fail=1
fi
